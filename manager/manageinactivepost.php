...
$sql = "SELECT Post.*, User.userName
        ...
        WHERE Post.archive = 0 and Post.inactive = 1
...
if ($result && $result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        if (isset($_SESSION['roleID']) && $_SESSION['roleID'] == 3) {
            echo '&nbsp<form method="POST" class = "manage-inactive-form" action = "processphp/processactive.php">
            <input type="hidden" name = "postID" value="' . $postID . '">
            <input type="hidden" name = "status" value="0">
            <button type="submit" class="archive-post-button active-button2">Click to Join Active</button>';
            echo '</form>';
            echo '&nbsp&nbsp<form method="POST" class = "manage-inactive-form" action = "processphp/processmanagearchive.php">
            <input type="hidden" name = "postID" value="' . $postID . '">
            <input type="hidden" name = "status" value="1">
            <button type="submit" class="archive-post-button active-post-button2">Click to Archive</button>';
            echo '</form>';
        }
